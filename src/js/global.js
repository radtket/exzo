...
function initSwiper() {
	$(".swiper-container")
		...
		.each(function() {
			...
			let slidesPerViewVar = $t.data("slides-per-view")
				? $t.data("slides-per-view")
				: 1;
			const loopVar = $t.data("loop") ? parseInt($t.data("loop"), 10) : 0;
			...
			swipers[`swiper-${index}`] = new Swiper(`.swiper-${index}`, {
				...
				preloadImages: false,
				lazyLoading: true,
				lazyLoadingInPrevNext: $t.data("direction") == "vertical",
...
if (!isMobile) {
	...
	window.addEventListener("orientationchange", resizeCall, false);
}
...
$(".hamburger-icon, .nav-close-layer").on("click", () => {
...
$(".toggle-search, .header-search-wrapper .button-close").on("click", () => {
...
$(document).on("click", ".open-popup-ajax", function(e) {
	...
	$.ajax({
		...
		url,
		success(response) {
			const responseObject = $($.parseHTML(response));
...
$(document).on("click", ".tabs-block .tab-menu", function() {
	...
	tabsFinish = 1;
	const tabsWrapper = $(this).closest(".tabs-block");
	const tabsMenu = tabsWrapper.find(".tab-menu");
	const tabsItem = tabsWrapper.find(".tab-entry");
...
$(".input-file-wrapper .file-remove").on("click", function() {
	const filewrapper = $(this).closest(".input-file-wrapper");
	const textwrapper = filewrapper.find(".simple-input");
